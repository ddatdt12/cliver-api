// <auto-generated />
using System;
using CliverApi.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace CliverApi.Migrations
{
    [DbContext(typeof(DataContext))]
    [Migration("20220723150614_SeedCategoryAndUser")]
    partial class SeedCategoryAndUser
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.6")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("CliverApi.Models.Category", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Category");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Graphics & Design"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Digital Marketing"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Writing & Translation"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Video & Animation"
                        },
                        new
                        {
                            Id = 5,
                            Name = "Music & Audio"
                        },
                        new
                        {
                            Id = 6,
                            Name = "Programming & Tech"
                        },
                        new
                        {
                            Id = 7,
                            Name = "Business"
                        },
                        new
                        {
                            Id = 8,
                            Name = "Lifestyle"
                        },
                        new
                        {
                            Id = 9,
                            Name = "Trending"
                        });
                });

            modelBuilder.Entity("CliverApi.Models.Package", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<bool>("CanContentUpload")
                        .HasColumnType("bit");

                    b.Property<bool>("CanDesignCustomized")
                        .HasColumnType("bit");

                    b.Property<int>("DeliveryTime")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("HasSourceCode")
                        .HasColumnType("bit");

                    b.Property<bool>("IsResponsiveDesign")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("NumberOfPages")
                        .HasColumnType("int");

                    b.Property<int?>("NumberOfRevisions")
                        .HasColumnType("int");

                    b.Property<int>("Price")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Package");
                });

            modelBuilder.Entity("CliverApi.Models.Post", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("BasicPackageId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Document")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("HasOfferPackages")
                        .HasColumnType("bit");

                    b.Property<string>("Images")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PremiumPackageId")
                        .HasColumnType("int");

                    b.Property<int?>("StandardPackageId")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("SubcategoryId")
                        .HasColumnType("int");

                    b.Property<string>("Tags")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Video")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("BasicPackageId");

                    b.HasIndex("PremiumPackageId");

                    b.HasIndex("StandardPackageId");

                    b.HasIndex("SubcategoryId");

                    b.HasIndex("UserId");

                    b.ToTable("Post");
                });

            modelBuilder.Entity("CliverApi.Models.Subcategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("CategoryId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CategoryId");

                    b.ToTable("Subcategory");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CategoryId = 1,
                            Name = "Social Media Marketing"
                        },
                        new
                        {
                            Id = 2,
                            CategoryId = 1,
                            Name = "Social Media Advertising"
                        },
                        new
                        {
                            Id = 3,
                            CategoryId = 1,
                            Name = "Search Engine Optimization (SEO)"
                        },
                        new
                        {
                            Id = 4,
                            CategoryId = 1,
                            Name = "Local SEO"
                        },
                        new
                        {
                            Id = 5,
                            CategoryId = 1,
                            Name = "Marketing Strategy"
                        },
                        new
                        {
                            Id = 6,
                            CategoryId = 1,
                            Name = "Public Relations"
                        },
                        new
                        {
                            Id = 7,
                            CategoryId = 1,
                            Name = "Guest Posting"
                        },
                        new
                        {
                            Id = 8,
                            CategoryId = 1,
                            Name = "Video Marketing"
                        },
                        new
                        {
                            Id = 9,
                            CategoryId = 1,
                            Name = "Email Marketing"
                        },
                        new
                        {
                            Id = 10,
                            CategoryId = 1,
                            Name = "Web Analytics"
                        },
                        new
                        {
                            Id = 11,
                            CategoryId = 1,
                            Name = "Text Message Marketing"
                        },
                        new
                        {
                            Id = 12,
                            CategoryId = 1,
                            Name = "Crowdfunding"
                        },
                        new
                        {
                            Id = 13,
                            CategoryId = 1,
                            Name = "Marketing Advice"
                        },
                        new
                        {
                            Id = 14,
                            CategoryId = 1,
                            Name = "Search Engine Marketing (SEM)"
                        },
                        new
                        {
                            Id = 15,
                            CategoryId = 1,
                            Name = "Display Advertising"
                        },
                        new
                        {
                            Id = 16,
                            CategoryId = 1,
                            Name = "E-Commerce Marketing"
                        },
                        new
                        {
                            Id = 17,
                            CategoryId = 1,
                            Name = "Influencer Marketing"
                        },
                        new
                        {
                            Id = 18,
                            CategoryId = 1,
                            Name = "Community Management"
                        },
                        new
                        {
                            Id = 19,
                            CategoryId = 1,
                            Name = "Mobile App Marketing"
                        },
                        new
                        {
                            Id = 20,
                            CategoryId = 1,
                            Name = "Music Promotion"
                        },
                        new
                        {
                            Id = 21,
                            CategoryId = 1,
                            Name = "Book & eBook Marketing"
                        },
                        new
                        {
                            Id = 22,
                            CategoryId = 1,
                            Name = "Podcast Marketing"
                        },
                        new
                        {
                            Id = 23,
                            CategoryId = 1,
                            Name = "Affiliate Marketing"
                        },
                        new
                        {
                            Id = 24,
                            CategoryId = 1,
                            Name = "Other"
                        },
                        new
                        {
                            Id = 25,
                            CategoryId = 2,
                            Name = "Logo Design"
                        },
                        new
                        {
                            Id = 26,
                            CategoryId = 2,
                            Name = "Brand Style Guides"
                        },
                        new
                        {
                            Id = 27,
                            CategoryId = 2,
                            Name = "Fonts & TypographyNEW"
                        },
                        new
                        {
                            Id = 28,
                            CategoryId = 2,
                            Name = "Business Cards & Stationery"
                        },
                        new
                        {
                            Id = 29,
                            CategoryId = 2,
                            Name = "Game Art"
                        },
                        new
                        {
                            Id = 30,
                            CategoryId = 2,
                            Name = "Graphics for Streamers"
                        },
                        new
                        {
                            Id = 31,
                            CategoryId = 2,
                            Name = "Twitch Store"
                        },
                        new
                        {
                            Id = 32,
                            CategoryId = 2,
                            Name = "Illustration"
                        },
                        new
                        {
                            Id = 33,
                            CategoryId = 2,
                            Name = "NFT Art"
                        },
                        new
                        {
                            Id = 34,
                            CategoryId = 2,
                            Name = "Pattern Design"
                        },
                        new
                        {
                            Id = 35,
                            CategoryId = 2,
                            Name = "Portraits & Caricatures"
                        },
                        new
                        {
                            Id = 36,
                            CategoryId = 2,
                            Name = "Cartoons & Comics"
                        },
                        new
                        {
                            Id = 37,
                            CategoryId = 2,
                            Name = "Tattoo Design"
                        },
                        new
                        {
                            Id = 38,
                            CategoryId = 2,
                            Name = "Storyboards"
                        },
                        new
                        {
                            Id = 39,
                            CategoryId = 2,
                            Name = "Website Design"
                        },
                        new
                        {
                            Id = 40,
                            CategoryId = 2,
                            Name = "App Design"
                        },
                        new
                        {
                            Id = 41,
                            CategoryId = 2,
                            Name = "UX Design"
                        },
                        new
                        {
                            Id = 42,
                            CategoryId = 2,
                            Name = "Landing Page Design"
                        },
                        new
                        {
                            Id = 43,
                            CategoryId = 2,
                            Name = "Icon Design"
                        },
                        new
                        {
                            Id = 44,
                            CategoryId = 2,
                            Name = "Social Media Design"
                        },
                        new
                        {
                            Id = 45,
                            CategoryId = 2,
                            Name = "Email Design"
                        },
                        new
                        {
                            Id = 46,
                            CategoryId = 2,
                            Name = "Web Banners"
                        },
                        new
                        {
                            Id = 47,
                            CategoryId = 2,
                            Name = "Signage Design"
                        },
                        new
                        {
                            Id = 48,
                            CategoryId = 2,
                            Name = "Packaging & Covers"
                        },
                        new
                        {
                            Id = 49,
                            CategoryId = 2,
                            Name = "Packaging & Label Design"
                        },
                        new
                        {
                            Id = 50,
                            CategoryId = 2,
                            Name = "Book Design"
                        },
                        new
                        {
                            Id = 51,
                            CategoryId = 2,
                            Name = "Album Cover Design"
                        },
                        new
                        {
                            Id = 52,
                            CategoryId = 2,
                            Name = "Podcast Cover Art"
                        },
                        new
                        {
                            Id = 53,
                            CategoryId = 2,
                            Name = "Car Wraps"
                        },
                        new
                        {
                            Id = 54,
                            CategoryId = 3,
                            Name = "Articles & Blog Posts"
                        },
                        new
                        {
                            Id = 55,
                            CategoryId = 3,
                            Name = "Translation"
                        },
                        new
                        {
                            Id = 56,
                            CategoryId = 3,
                            Name = "Proofreading & Editing"
                        },
                        new
                        {
                            Id = 57,
                            CategoryId = 3,
                            Name = "Resume Writing"
                        },
                        new
                        {
                            Id = 58,
                            CategoryId = 3,
                            Name = "Cover Letters"
                        },
                        new
                        {
                            Id = 59,
                            CategoryId = 3,
                            Name = "LinkedIn Profiles"
                        },
                        new
                        {
                            Id = 60,
                            CategoryId = 3,
                            Name = "Ad Copy"
                        },
                        new
                        {
                            Id = 61,
                            CategoryId = 3,
                            Name = "Sales Copy"
                        },
                        new
                        {
                            Id = 62,
                            CategoryId = 3,
                            Name = "Social Media Copy"
                        },
                        new
                        {
                            Id = 63,
                            CategoryId = 3,
                            Name = "Email Copy"
                        },
                        new
                        {
                            Id = 64,
                            CategoryId = 3,
                            Name = "Case Studies"
                        },
                        new
                        {
                            Id = 65,
                            CategoryId = 3,
                            Name = "Book & eBook Writing"
                        },
                        new
                        {
                            Id = 66,
                            CategoryId = 3,
                            Name = "Book Editing"
                        },
                        new
                        {
                            Id = 67,
                            CategoryId = 3,
                            Name = "Scriptwriting"
                        },
                        new
                        {
                            Id = 68,
                            CategoryId = 3,
                            Name = "Podcast Writing"
                        },
                        new
                        {
                            Id = 69,
                            CategoryId = 3,
                            Name = "Beta Reading"
                        },
                        new
                        {
                            Id = 70,
                            CategoryId = 3,
                            Name = "Creative Writing"
                        },
                        new
                        {
                            Id = 71,
                            CategoryId = 3,
                            Name = "Brand Voice & Tone"
                        },
                        new
                        {
                            Id = 72,
                            CategoryId = 3,
                            Name = "UX Writing"
                        },
                        new
                        {
                            Id = 73,
                            CategoryId = 3,
                            Name = "Speechwriting"
                        },
                        new
                        {
                            Id = 74,
                            CategoryId = 3,
                            Name = "eLearning Content Development"
                        },
                        new
                        {
                            Id = 75,
                            CategoryId = 3,
                            Name = "Technical Writing"
                        },
                        new
                        {
                            Id = 76,
                            CategoryId = 4,
                            Name = "Video Editing"
                        },
                        new
                        {
                            Id = 77,
                            CategoryId = 4,
                            Name = "Short Video Ads"
                        },
                        new
                        {
                            Id = 78,
                            CategoryId = 4,
                            Name = "Whiteboard & Animated Explainers"
                        },
                        new
                        {
                            Id = 79,
                            CategoryId = 4,
                            Name = "Character Animation"
                        },
                        new
                        {
                            Id = 80,
                            CategoryId = 4,
                            Name = "Lyric & Music Videos"
                        },
                        new
                        {
                            Id = 81,
                            CategoryId = 4,
                            Name = "Logo Animation"
                        },
                        new
                        {
                            Id = 82,
                            CategoryId = 4,
                            Name = "Intros & Outros"
                        },
                        new
                        {
                            Id = 83,
                            CategoryId = 4,
                            Name = "Visual Effects"
                        },
                        new
                        {
                            Id = 84,
                            CategoryId = 4,
                            Name = "Subtitles & Captions"
                        },
                        new
                        {
                            Id = 85,
                            CategoryId = 4,
                            Name = "Spokesperson Videos"
                        },
                        new
                        {
                            Id = 86,
                            CategoryId = 4,
                            Name = "Unboxing Videos"
                        },
                        new
                        {
                            Id = 87,
                            CategoryId = 4,
                            Name = "Animated GIFs"
                        },
                        new
                        {
                            Id = 88,
                            CategoryId = 5,
                            Name = "Voice Over"
                        },
                        new
                        {
                            Id = 89,
                            CategoryId = 5,
                            Name = "Producers & Composers"
                        },
                        new
                        {
                            Id = 90,
                            CategoryId = 5,
                            Name = "Singers & Vocalists"
                        },
                        new
                        {
                            Id = 91,
                            CategoryId = 5,
                            Name = "Mixing & Mastering"
                        },
                        new
                        {
                            Id = 92,
                            CategoryId = 5,
                            Name = "Session Musicians"
                        },
                        new
                        {
                            Id = 93,
                            CategoryId = 5,
                            Name = "Online Music Lessons"
                        },
                        new
                        {
                            Id = 94,
                            CategoryId = 5,
                            Name = "Podcast Editing"
                        },
                        new
                        {
                            Id = 95,
                            CategoryId = 5,
                            Name = "Songwriters"
                        },
                        new
                        {
                            Id = 96,
                            CategoryId = 5,
                            Name = "Beat Making"
                        },
                        new
                        {
                            Id = 97,
                            CategoryId = 5,
                            Name = "Audiobook Production"
                        },
                        new
                        {
                            Id = 98,
                            CategoryId = 5,
                            Name = "Audio Ads Production"
                        },
                        new
                        {
                            Id = 99,
                            CategoryId = 5,
                            Name = "Sound Design"
                        },
                        new
                        {
                            Id = 100,
                            CategoryId = 6,
                            Name = "WordPress"
                        },
                        new
                        {
                            Id = 101,
                            CategoryId = 6,
                            Name = "Website Builders & CMS"
                        },
                        new
                        {
                            Id = 102,
                            CategoryId = 6,
                            Name = "Game Development"
                        },
                        new
                        {
                            Id = 103,
                            CategoryId = 6,
                            Name = "Development for Streamers"
                        },
                        new
                        {
                            Id = 104,
                            CategoryId = 6,
                            Name = "Web Programming"
                        },
                        new
                        {
                            Id = 105,
                            CategoryId = 6,
                            Name = "E-Commerce Development"
                        },
                        new
                        {
                            Id = 106,
                            CategoryId = 6,
                            Name = "Mobile Apps"
                        },
                        new
                        {
                            Id = 107,
                            CategoryId = 6,
                            Name = "Desktop Applications"
                        },
                        new
                        {
                            Id = 108,
                            CategoryId = 6,
                            Name = "Chatbots"
                        },
                        new
                        {
                            Id = 109,
                            CategoryId = 6,
                            Name = "Support & IT"
                        },
                        new
                        {
                            Id = 110,
                            CategoryId = 6,
                            Name = "Online Coding Lessons"
                        },
                        new
                        {
                            Id = 111,
                            CategoryId = 6,
                            Name = "Cybersecurity & Data Protection"
                        },
                        new
                        {
                            Id = 112,
                            CategoryId = 6,
                            Name = "Electronics Engineering"
                        },
                        new
                        {
                            Id = 113,
                            CategoryId = 6,
                            Name = "Convert Files"
                        },
                        new
                        {
                            Id = 114,
                            CategoryId = 6,
                            Name = "User Testing"
                        },
                        new
                        {
                            Id = 115,
                            CategoryId = 6,
                            Name = "QA & Review"
                        },
                        new
                        {
                            Id = 116,
                            CategoryId = 6,
                            Name = "Blockchain & Cryptocurrency"
                        },
                        new
                        {
                            Id = 117,
                            CategoryId = 6,
                            Name = "NFT Development"
                        },
                        new
                        {
                            Id = 118,
                            CategoryId = 6,
                            Name = "Databases"
                        },
                        new
                        {
                            Id = 119,
                            CategoryId = 6,
                            Name = "Data Processing"
                        },
                        new
                        {
                            Id = 120,
                            CategoryId = 6,
                            Name = "Data Engineering"
                        },
                        new
                        {
                            Id = 121,
                            CategoryId = 6,
                            Name = "Data Science"
                        },
                        new
                        {
                            Id = 122,
                            CategoryId = 6,
                            Name = "Other"
                        },
                        new
                        {
                            Id = 123,
                            CategoryId = 6,
                            Name = ""
                        },
                        new
                        {
                            Id = 124,
                            CategoryId = 7,
                            Name = "Virtual Assistant"
                        },
                        new
                        {
                            Id = 125,
                            CategoryId = 7,
                            Name = "E-Commerce Management"
                        },
                        new
                        {
                            Id = 126,
                            CategoryId = 7,
                            Name = "Market Research"
                        },
                        new
                        {
                            Id = 127,
                            CategoryId = 7,
                            Name = "Sales"
                        },
                        new
                        {
                            Id = 128,
                            CategoryId = 7,
                            Name = "Customer Care"
                        },
                        new
                        {
                            Id = 129,
                            CategoryId = 7,
                            Name = "CRM Management NEW"
                        },
                        new
                        {
                            Id = 130,
                            CategoryId = 7,
                            Name = "ERP ManagementNEW"
                        },
                        new
                        {
                            Id = 131,
                            CategoryId = 7,
                            Name = "Supply Chain Management"
                        },
                        new
                        {
                            Id = 132,
                            CategoryId = 7,
                            Name = "Project Management"
                        },
                        new
                        {
                            Id = 133,
                            CategoryId = 7,
                            Name = "Event ManagementNEW"
                        },
                        new
                        {
                            Id = 134,
                            CategoryId = 7,
                            Name = "Game Concept Design"
                        },
                        new
                        {
                            Id = 135,
                            CategoryId = 7,
                            Name = "Business Plans"
                        },
                        new
                        {
                            Id = 136,
                            CategoryId = 7,
                            Name = "Financial Consulting"
                        },
                        new
                        {
                            Id = 137,
                            CategoryId = 7,
                            Name = "Legal Consulting"
                        },
                        new
                        {
                            Id = 138,
                            CategoryId = 7,
                            Name = "Business Consulting"
                        },
                        new
                        {
                            Id = 139,
                            CategoryId = 7,
                            Name = "Presentations"
                        },
                        new
                        {
                            Id = 140,
                            CategoryId = 7,
                            Name = "HR Consulting"
                        },
                        new
                        {
                            Id = 141,
                            CategoryId = 7,
                            Name = "Career Counseling"
                        },
                        new
                        {
                            Id = 142,
                            CategoryId = 7,
                            Name = "Data Entry"
                        },
                        new
                        {
                            Id = 143,
                            CategoryId = 7,
                            Name = "Data Analytics"
                        },
                        new
                        {
                            Id = 144,
                            CategoryId = 7,
                            Name = "Data Visualization"
                        },
                        new
                        {
                            Id = 145,
                            CategoryId = 7,
                            Name = "Other"
                        },
                        new
                        {
                            Id = 146,
                            CategoryId = 8,
                            Name = "Online Tutoring"
                        },
                        new
                        {
                            Id = 147,
                            CategoryId = 8,
                            Name = "Gaming"
                        },
                        new
                        {
                            Id = 148,
                            CategoryId = 8,
                            Name = "Astrology & Psychics"
                        },
                        new
                        {
                            Id = 149,
                            CategoryId = 8,
                            Name = "Modeling & Acting"
                        },
                        new
                        {
                            Id = 150,
                            CategoryId = 8,
                            Name = "Wellness"
                        },
                        new
                        {
                            Id = 151,
                            CategoryId = 8,
                            Name = "Traveling"
                        },
                        new
                        {
                            Id = 152,
                            CategoryId = 8,
                            Name = "Fitness Lessons"
                        },
                        new
                        {
                            Id = 153,
                            CategoryId = 8,
                            Name = "Dance Lessons"
                        },
                        new
                        {
                            Id = 154,
                            CategoryId = 8,
                            Name = "Life Coaching"
                        },
                        new
                        {
                            Id = 155,
                            CategoryId = 8,
                            Name = "Greeting Cards & Videos"
                        },
                        new
                        {
                            Id = 156,
                            CategoryId = 8,
                            Name = "Personal Stylists"
                        },
                        new
                        {
                            Id = 157,
                            CategoryId = 8,
                            Name = "Cooking Lessons"
                        },
                        new
                        {
                            Id = 158,
                            CategoryId = 8,
                            Name = "Craft Lessons"
                        },
                        new
                        {
                            Id = 159,
                            CategoryId = 8,
                            Name = "Arts & Crafts"
                        },
                        new
                        {
                            Id = 160,
                            CategoryId = 8,
                            Name = "Family & Genealogy"
                        },
                        new
                        {
                            Id = 161,
                            CategoryId = 8,
                            Name = "Collectibles"
                        },
                        new
                        {
                            Id = 162,
                            CategoryId = 8,
                            Name = "Other"
                        },
                        new
                        {
                            Id = 163,
                            CategoryId = 9,
                            Name = "Dropshipping"
                        },
                        new
                        {
                            Id = 164,
                            CategoryId = 9,
                            Name = "E-Commerce Marketing"
                        },
                        new
                        {
                            Id = 165,
                            CategoryId = 9,
                            Name = "Game Development"
                        },
                        new
                        {
                            Id = 166,
                            CategoryId = 9,
                            Name = "Discord Services"
                        },
                        new
                        {
                            Id = 167,
                            CategoryId = 9,
                            Name = "NFT Services"
                        },
                        new
                        {
                            Id = 168,
                            CategoryId = 9,
                            Name = "Architecture & Interior Design"
                        },
                        new
                        {
                            Id = 169,
                            CategoryId = 9,
                            Name = "Data"
                        },
                        new
                        {
                            Id = 170,
                            CategoryId = 9,
                            Name = "Resume Writing"
                        },
                        new
                        {
                            Id = 171,
                            CategoryId = 9,
                            Name = "Search Engine Optimization (SEO)"
                        },
                        new
                        {
                            Id = 172,
                            CategoryId = 9,
                            Name = "Character Modeling"
                        },
                        new
                        {
                            Id = 173,
                            CategoryId = 9,
                            Name = "Character Animation"
                        },
                        new
                        {
                            Id = 174,
                            CategoryId = 9,
                            Name = "Image Editing"
                        });
                });

            modelBuilder.Entity("CliverApi.Models.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<long>("AvailableForWithdrawal")
                        .HasColumnType("bigint");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("ExpectedEarnings")
                        .HasColumnType("bigint");

                    b.Property<bool>("IsActived")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("NetIncome")
                        .HasColumnType("bigint");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("PendingClearance")
                        .HasColumnType("bigint");

                    b.Property<int>("Type")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<long>("UsedForPurchases")
                        .HasColumnType("bigint");

                    b.Property<long>("Withdrawn")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("User");

                    b.HasData(
                        new
                        {
                            Id = "2da0c54d-afc3-475d-830c-78cac920b4a4",
                            AvailableForWithdrawal = 0L,
                            CreatedAt = new DateTime(2022, 7, 23, 22, 6, 14, 672, DateTimeKind.Local).AddTicks(7630),
                            Description = "",
                            Email = "test@gmail.com",
                            ExpectedEarnings = 0L,
                            IsActived = true,
                            Name = "Test 1",
                            NetIncome = 0L,
                            Password = "123123123",
                            PendingClearance = 0L,
                            Type = 1,
                            UpdatedAt = new DateTime(2022, 7, 23, 22, 6, 14, 672, DateTimeKind.Local).AddTicks(7660),
                            UsedForPurchases = 0L,
                            Withdrawn = 0L
                        },
                        new
                        {
                            Id = "48eb824c-dbed-4b74-b1f7-9b84a68ae45a",
                            AvailableForWithdrawal = 0L,
                            CreatedAt = new DateTime(2022, 7, 23, 22, 6, 14, 672, DateTimeKind.Local).AddTicks(7665),
                            Description = "",
                            Email = "admin@admin.com",
                            ExpectedEarnings = 0L,
                            IsActived = true,
                            Name = "admin",
                            NetIncome = 0L,
                            Password = "123123123",
                            PendingClearance = 0L,
                            Type = 0,
                            UpdatedAt = new DateTime(2022, 7, 23, 22, 6, 14, 672, DateTimeKind.Local).AddTicks(7665),
                            UsedForPurchases = 0L,
                            Withdrawn = 0L
                        });
                });

            modelBuilder.Entity("CliverApi.Models.Post", b =>
                {
                    b.HasOne("CliverApi.Models.Package", "BasicPackage")
                        .WithMany()
                        .HasForeignKey("BasicPackageId");

                    b.HasOne("CliverApi.Models.Package", "PremiumPackage")
                        .WithMany()
                        .HasForeignKey("PremiumPackageId");

                    b.HasOne("CliverApi.Models.Package", "StandardPackage")
                        .WithMany()
                        .HasForeignKey("StandardPackageId");

                    b.HasOne("CliverApi.Models.Subcategory", "Subcategory")
                        .WithMany()
                        .HasForeignKey("SubcategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("CliverApi.Models.User", "User")
                        .WithMany("Posts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BasicPackage");

                    b.Navigation("PremiumPackage");

                    b.Navigation("StandardPackage");

                    b.Navigation("Subcategory");

                    b.Navigation("User");
                });

            modelBuilder.Entity("CliverApi.Models.Subcategory", b =>
                {
                    b.HasOne("CliverApi.Models.Category", "Category")
                        .WithMany("Subcategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Category");
                });

            modelBuilder.Entity("CliverApi.Models.Category", b =>
                {
                    b.Navigation("Subcategories");
                });

            modelBuilder.Entity("CliverApi.Models.User", b =>
                {
                    b.Navigation("Posts");
                });
#pragma warning restore 612, 618
        }
    }
}
